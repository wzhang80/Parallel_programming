...
package object barneshut {
  ...
  extends Quad {
    ...
    val (mass, massX, massY) = (m : Float, bodies.map(b => b.mass * b.x).sum / m : Float, bodies.map(b => b.mass * b.y).sum / m : Float)
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        (bodies :+ b).foldLeft(Fork(nw, ne, sw, se))((acc, body) => acc.insert(body))
      } else {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      this.apply(((x - boundaries.minX)/sectorSize).toInt, ((y - boundaries.minY)/sectorSize).toInt) += b
      this
    }
